"""
============================================================
  Train a Fourier Neural Operator on PFC / Kobayashi trajectories

  Objective:  minimize  WeightedMSE( FNO(n_t) , n_t+1 )
  Uses a crystal-weighted loss that penalises errors on solid
  pixels (p > 0.1) much more heavily than liquid pixels,
  preventing the model from learning to predict all-zeros
  on sparse Kobayashi fields.
...
The checkpoint stores the model weights AND the normalization
stats + config, so evaluate_fno.py can reproduce the exact
preprocessing.
"""
...
class WeightedMSE(nn.Module):
    """
    Crystal-weighted MSE loss for sparse phase fields.

    Standard MSE fails on Kobayashi data because the field is ~95% liquid
    (value ≈ 0). The model can achieve very low MSE by predicting all-zeros,
    which produces no crystal growth at all.

    This loss puts `crystal_weight` times more penalty on pixels where the
    TARGET is solid (> threshold), forcing the model to actually learn the
    crystal dynamics rather than the trivial all-liquid prediction.

    crystal_weight=50 means: getting a crystal pixel wrong costs 50x more
    than getting a liquid pixel wrong.

    For PFC data (dense fields, normalize=true) set crystal_weight=1.0
    to recover standard MSE.
    """
    def __init__(self, crystal_weight: float = 50.0, threshold: float = 0.1):
        super().__init__()
        self.w         = crystal_weight
        self.threshold = threshold

    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        # weights: 1.0 for liquid, crystal_weight for solid
        weights = 1.0 + (self.w - 1.0) * (target > self.threshold).float()
...
def run_epoch(model, loader, loss_fn, device, optimizer=None,
    ...
    If `optimizer` is given we train; otherwise evaluate under no_grad.
    Returns the mean weighted loss.
    """
...
def main():
    ...
    print(f"Normalization -> mean {info['norm_mean']:.5f}  "
          f"std {info['norm_std']:.5f}")
    ...
    print(f"Model: FNO2d  modes={cfg['model']['modes']} "
          f"width={cfg['model']['width']} "
          f"layers={cfg['model']['layers']}  |  "
    ...
    # Crystal-weighted MSE — essential for sparse Kobayashi fields.
    # For dense PFC fields set crystal_weight=1.0 in config or here.
    crystal_weight = cfg["train"].get("crystal_weight", 50.0)
    loss_fn        = WeightedMSE(crystal_weight=crystal_weight)
    print(f"Loss: WeightedMSE  crystal_weight={crystal_weight}")
    ...
    for epoch in range(es["epochs"]):
        ...
        val_loss = run_epoch(model, val_loader, loss_fn, device, tag="val")
        ...
        dt       = time.time() - t0
        improved = val_loss < best_val - es.get("min_delta", 0.0)
        flag     = ""

        if improved:
            best_val          = val_loss
            epochs_no_improve = 0
            save_ckpt(ckpt_path, epoch, val_loss)
            flag = "  <- best (saved)"
        else:
            epochs_no_improve += 1

        save_ckpt(last_path, epoch, val_loss)
        ...
        # Early stopping
        if es.get("early_stopping", False) \
                and epochs_no_improve >= es["patience"]:
            print(f"\nEarly stopping: no val improvement for "
                  f"{es['patience']} epochs.")
            break

    if writer is not None:
        writer.close()

    print(f"\nDone. Best val loss: {best_val:.3e}")
    print(f"Best checkpoint:  {ckpt_path}")
    print(f"Last checkpoint:  {last_path}")
    print(f"Evaluate with:  python evaluate_fno.py --config config.yaml "
